...
public class ManufacturingReportByEmployeeWordService
{
     public static byte[] Generate(List<ManufacturingReportByEmployeeDto> data, ProductManufacturingTotalDto totalData, string directorName, string user, string role, DateTime? startDate = null, DateTime? endDate = null)
    {
        ...
        doc.InsertParagraph("Manufacturing Report by Employees").FontSize(16).Bold().Alignment = Alignment.center;
        ...
        var table = doc.AddTable(data.Count + 2, 3);
        ...
        for (int i = 0; i < data.Count; i++)
        {
            ...
            row.Cells[1].Paragraphs[0].Append(item.FullName);
