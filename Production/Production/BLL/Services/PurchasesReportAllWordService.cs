...
public class PurchasesReportAllWordService
{
    public static byte[] Generate(List<RawMaterialPurchasesHistoryDto> data, ProductSaleTotalDto totalData, string directorName, string user, string role, DateTime? startDate = null, DateTime? endDate = null)
    {
        ...
        doc.InsertParagraph("Purchases Report All").FontSize(16).Bold().Alignment = Alignment.center;
        ...
        var table = doc.AddTable(data.Count + 2, 6);
        table.Design = TableDesign.TableGrid;
        
        table.Rows[0].Cells[0].Paragraphs[0].Append("№").Bold();
        table.Rows[0].Cells[1].Paragraphs[0].Append("Raw Material").Bold();;
        table.Rows[0].Cells[2].Paragraphs[0].Append("Purchase Date").Bold();;
        table.Rows[0].Cells[3].Paragraphs[0].Append("Employee").Bold();;
        table.Rows[0].Cells[4].Paragraphs[0].Append("Quantity").Bold();;
        table.Rows[0].Cells[5].Paragraphs[0].Append("Amount").Bold();;
        ...
        for (int i = 0; i < data.Count; i++)
        {
            ...
            row.Cells[2].Paragraphs[0].Append(item.PurchaseDate.ToString());
            row.Cells[3].Paragraphs[0].Append(item.FullName);
            row.Cells[4].Paragraphs[0].Append(item.Quantity.ToString("F2"));
            row.Cells[5].Paragraphs[0].Append(item.TotalAmount.ToString("F2"));
        }
        ...
        table.Rows[data.Count + 1].Cells[5].Paragraphs[0].Append(total.TotalSaleAmount.ToString()).Bold();;
