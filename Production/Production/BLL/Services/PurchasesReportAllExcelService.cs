...
public static class PurchasesReportAllExcelService
{
   public static byte[] Generate(List<RawMaterialPurchasesHistoryDto> data, ProductSaleTotalDto totalData)
{
    using var workbook = new XLWorkbook();
    var worksheet = workbook.Worksheets.Add("Sales Report All");
    
    worksheet.Cell(1, 1).Value = "№";
    worksheet.Cell(1, 2).Value = "Raw Material";
    ...
    for (int i = 0; i < data.Count; i++)
    {
        ...
        worksheet.Cell(i + 2, 3).Value = item.PurchaseDate.ToString();
