...
public static class ManufacturingReportAllExcelService
{
   public static byte[] Generate(List<ProductManufacturingHistoryDto> data, ProductManufacturingTotalDto totalData)
{
    ...
    worksheet.Cell(1, 3).Value = "Manufacturing date";
    ...
    for (int i = 0; i < data.Count; i++)
    {
        ...
        worksheet.Cell(i + 2, 3).Value = item.ManufacturingDate.ToString();
        worksheet.Cell(i + 2, 4).Value = item.FullName;
        worksheet.Cell(i + 2, 5).Value = item.Quantity.ToString("F2");
    }

    var dataRange = worksheet.Range(2, 1, data.Count + 1, 5);
    ...
    var totalHeader = worksheet.Range(totalRow + 1, 2, totalRow + 1, 5);
    ...
    var totalValues = worksheet.Range(totalRow + 1, 1, totalRow + 1, 5);
