...
public class PurchasesReportAllProductPdfService : IDocument
{
    private readonly List<RawMaterialPurchasesHistoryDto> _data;
    private ProductSaleTotalDto _totalData;
    ...
    public PurchasesReportAllProductPdfService(List<RawMaterialPurchasesHistoryDto> data, ProductSaleTotalDto totalData, string directorName, string username, string role, DateTime? startDate = null, DateTime? endDate = null)
    {
        _data = data;
    ...
    void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().Text("Purchases Report All").FontSize(18).Bold().AlignCenter();
        ...
        container.PaddingTop(15).Column(column =>
        {
            ...
            column.Item().Table(table =>
            {
                ...
                table.Header(header =>
                {
                    ...
                    header.Cell().Element(CellStyle).AlignCenter().Text("Raw Material").SemiBold();
                    header.Cell().Element(CellStyle).AlignCenter().Text("Purchase Date").SemiBold();
                ...
                foreach (var item in _data)
                {
                    ...
                    table.Cell().Element(CellStyle).AlignCenter().Text(item.PurchaseDate);
                    ...
                    table.Cell().Element(CellStyle).AlignCenter().Text(item.TotalAmount.ToString("F2"));
                    i++;
                }
                var totalQuantity = _totalData.TotalQuantity;
                var totalAmount = _totalData.TotalSaleAmount;
                ...
                table.Cell().Element(CellStyle).Text(totalAmount.ToString()).SemiBold().AlignCenter();
                table.Cell(); 
            });
